...
const Dashboard = () => {
  ...
  const fetchHistory = async () => {
    try {
      const res = await axios.get("http://sheshield-api.onrender.com/api/sos/history/" + user.id);
  ...
  const handleSOS = async () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        startRecording();
        try {
          await axios.post("http://sheshield-api.onrender.com/api/sos/send", { userId: user.id, location: loc });
          fetchHistory();
        } catch (err) {
          console.log(err);
        }
  ...
  const handleAddContact = async () => {
    ...
    try {
      const res = await axios.post("http://sheshield-api.onrender.com/api/contacts/add", {
        userId: user.id,
        name: contact.name,
        phone: contact.phone,
      });
  ...
  const handleDeleteContact = async (contactId) => {
    try {
      const res = await axios.delete("http://sheshield-api.onrender.com/api/contacts/" + user.id + "/" + contactId);
      setContacts(res.data.contacts);
  ...
  return (
    <div style={{ minHeight: "100vh", background: bg, transition: "0.3s", fontFamily: "'Segoe UI', sans-serif" }}>
      ...
      <div style={{ display: "flex", overflowX: "auto", padding: "15px 20px", gap: "10px", background: cardBg, borderBottom: "1px solid #f0f0f0" }}>
        {["sos", "location", "contacts", "history", "nearby", "notifications", "analytics"].map((tab) => (
